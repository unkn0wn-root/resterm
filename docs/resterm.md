...
Generated requests use `recordedBaseUrl`. Change it to the mock server address for replay. Mocks match the method, path, query values that were not redacted, and JSON fields that were not redacted. Form and text bodies are not used for matching. When mocks match the same request, the first one wins. Use `X-Resterm-Mock: <scenario-name>` to select a particular response. Requests are named from the method and path, such as `get-users-42`, and carry a description with the capture ID, status, and timing. Each request asserts the recorded status code, except redirects, since `resterm run` follows them. Resterm does not create response sequences. Add more `@assert` and `@expect` rules as needed, then use `resterm run` and `resterm mock verify`.
