...
| `[status_bar]` | `base`, plus `[status_bar.info]`, `[status_bar.warn]`, `[status_bar.error]`, `[status_bar.success]`, `[status_bar.tests]`, `[status_bar.file]`, `[status_bar.focus]`, `[status_bar.mode]`, `[status_bar.editor]`, `[status_bar.zoom]`, `[status_bar.minimized]`, `[status_bar.version]`, `[status_bar.user]`, `[status_bar.host]` with `foreground` and `background` | Controls the segmented bottom status bar. Segment entries inherit the built-in palette. `[status_bar.tests]` colours successful test-result blocks separately from request success; failed tests still use warning/error colours. `[status_bar.editor]` colours the editor cursor position (`Ln/Col`), shown faint by default; set it to override. `base` fills otherwise empty status-bar cells when set; omit it to leave them uncoloured. |
