...
| `[[command_segments]]` | `background`, `border`, `key`, `text` | Colour sets for command bar hints. Contextual keys render as keycap chips on `background`; clearing `background` renders that segment flat. Pinned hints use only `key` and `text`. `border` is accepted for compatibility. |
