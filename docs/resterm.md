...
| `[[command_segments]]` | `background`, `border`, `key`, `text` | Colour sets for command bar hints. Hints render flat, so only `key` and `text` apply; `background` and `border` are accepted for compatibility. |
