...
- A `--source` scope is remembered like the address, so `:mock restart`, a later `:mock start`, and `g Shift+M` keep serving the same files. `--all` and `--recursive` are the way back to the whole workspace, and `:mock status` names the remembered scope while the server is stopped. Changing the workspace forgets it.
