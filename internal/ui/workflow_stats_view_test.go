...
func TestWorkflowStatsJumpSelectionAlignsExpandedEntries(t *testing.T) {
	...
	state := &workflowState{
		...
		results: []workflowStepResult{
			workflowStepResult{Step: restfile.WorkflowStep{Name: "Step 1"}, Success: true, HTTP: &httpclient.Response{Status: "200 OK", StatusCode: 200, Body: []byte(strings.Repeat("one\n", 10))}},
			workflowStepResult{Step: restfile.WorkflowStep{Name: "Step 2"}, Success: true, HTTP: &httpclient.Response{Status: "200 OK", StatusCode: 200, Body: []byte(strings.Repeat("two\n", 10))}},
			workflowStepResult{Step: restfile.WorkflowStep{Name: "Step 3"}, Success: true, HTTP: &httpclient.Response{Status: "200 OK", StatusCode: 200, Body: []byte(strings.Repeat("three\n", 16))}},
			workflowStepResult{Step: restfile.WorkflowStep{Name: "Step 4"}, Success: true, HTTP: &httpclient.Response{Status: "200 OK", StatusCode: 200, Body: []byte(strings.Repeat("four\n", 20))}},
		},
		start: time.Now(),
		end:   time.Now(),
	}
