...
func (m *Model) executeRequest(doc *restfile.Document, req *restfile.Request, options httpclient.Options, envOverride string, extras ...map[string]string) tea.Cmd {
	...
	return func() tea.Msg {
		...
		globalSettings := settingsFromEnv(m.cfg.EnvironmentSet, envName)
		fileSettings := map[string]string{}
		if doc != nil && doc.Settings != nil {
			fileSettings = doc.Settings
		}
		mergedSettings := mergeSettings(globalSettings, fileSettings, req.Settings)
		...
		if err := applyHTTPSettings(&options, mergedSettings, resolver); err != nil {
			return responseMsg{err: err, executed: req}
		}
		...
		if req.GRPC != nil {
			...
			if err := applyGRPCSettings(&grpcOpts, mergedSettings, resolver); err != nil {
				return responseMsg{err: err, executed: req}
			}
...
func settingsFromEnv(set vars.EnvironmentSet, envName string) map[string]string {
...
func mergeSettings(scopes ...map[string]string) map[string]string {
	out := make(map[string]string)
