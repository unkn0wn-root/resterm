...
func TestApplyThemeDefinitionStylesFiltersUseDistinctPromptAndTextColors(t *testing.T) {
	...
	if got := model.commandLine.input.PromptStyle.GetForeground(); got != lipgloss.Color("#1e40af") {
		t.Fatalf("expected light command prompt foreground, got %v", got)
	}
	if got := model.commandLine.input.TextStyle.GetForeground(); got != lipgloss.Color("#0f172a") {
		t.Fatalf("expected light command text foreground, got %v", got)
	}
	if got := model.commandLine.input.Cursor.Style.GetForeground(); got != lipgloss.Color("#1e40af") {
