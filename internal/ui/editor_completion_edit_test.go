...
func TestRequestEditorCompletionPreservesExistingText(t *testing.T) {
	cases := []struct {
		...
		{"expression operator", `GET {{= ho|st + "/v1" }}`, "host", `GET {{= host + "/v1" }}`},
		{"expression parentheses", "GET {{= (ho|) }}", "host", "GET {{= (host) }}"},
		{"expression list", "GET {{= [ho|][0] }}", "host", "GET {{= [host][0] }}"},
		{"expression argument", "GET {{= string(ho|) }}", "host", "GET {{= string(host) }}"},
		{"unfinished expression", `GET {{= ho| + "/v1"`, "host", `GET {{= host + "/v1"`},
