...
func (r Runner) RunHTTP(in HTTPInput) HTTPResult {
	if in.Client == nil {
		return HTTPResult{
			Err:      errdef.New(errdef.CodeHTTP, "http client is not initialised"),
	...
	if in.Req == nil {
		return HTTPResult{
			Err:      errdef.New(errdef.CodeHTTP, "request is nil"),
			Decision: "HTTP request failed",
		}
	}
	...
	if streamErr != nil {
		res.Err = errdef.Wrap(errdef.CodeHTTP, streamErr, "decode stream transcript")
