...
func TestShorthandAfterBodyBecomesFileScopeWithoutTerminator(t *testing.T) {
	src := `### reset
...
`
	...
	if len(req.Variables) != 0 {
		t.Fatalf("expected no request variables, got %d", len(req.Variables))
	}
	...
	for _, v := range doc.Variables {
		...
		if v.Scope != restfile.ScopeFile {
			t.Fatalf("expected %s to be file scoped, got %v", v.Name, v.Scope)
		}
	}
	if names["tail"] != "outside" || names["tail2"] != "outside-2" {
		t.Fatalf("unexpected file vars: %+v", names)
	}
}

func TestShorthandAfterHeaderWithoutBodyBecomesFileScope(t *testing.T) {
...
`
	...
	v := doc.Variables[0]
	if v.Name != "tail" || v.Value != "outside" || v.Scope != restfile.ScopeFile {
		t.Fatalf("unexpected file var %+v", v)
	}
}
